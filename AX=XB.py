...
def get_RT_from_chessboard(img_path,chess_board_x_num,chess_board_y_num,K,chess_board_len):
    ...
    cv2.imshow('Demo',img)
    cv2.waitKey(0)
    corner_points=np.zeros((2,corners.shape[0]),dtype=np.float64) #构建行数为3，列数为角点数的零矩阵
    for i in range(corners.shape[0]):
        corner_points[:,i]=corners[i,0,:] #得到了一个矩阵每一列代表了角点的坐标
...
folder = r"C:\Users\Aliya\Desktop\calib" #棋盘格图片存放文件夹

files = os.listdir(folder)
file_num=len(files)
RT_all=np.zeros((4,4,file_num))
#print(get_RT_from_chessboard(r"C:\Users\Aliya\Desktop\calib\picture2.jpg", chess_board_x_num, chess_board_y_num, K, chess_board_len))

'''
这个地方很奇怪的特点，有些棋盘格点检测得出来，有些检测不了，可以通过函数get_RT_from_chessboard的运行时间来判断
'''
good_picture=[1,2,3] #存放可以检测出棋盘格角点的图片
...
#计算end to base变换矩阵
file_address=r"C:\Users\Aliya\Desktop\calib\position.xlsx"#从记录文件读取机器人六个位姿
